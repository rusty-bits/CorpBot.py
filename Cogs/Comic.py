...
class Comic(commands.Cog):
    ...
    def __init__(self, bot, settings):
        ...
        self.comic_data = {
            ...
            "xkcd": {
                ...
                "date_url": [
                            {"find": 'title="{}-{}-{}"',
                                "keys": ["year", "month", "day"], "index":0},
                            {"find": 'a href="/', "index": -1},
                            {"find": '/"', "index": 0}
                        ],
                "url": "https://xkcd.com/{}/",
                "comic_url": [
                            {"find": "(for hotlinking/embedding): ",
                             "index": 1},
                            {"find": "\n", "index": 0}
                        ],
                "comic_desc": [
    ...
    def _date_dict(self, date, padded=True):
        ...
        m, d, y = [str(int(x)).rjust(2, "0") if padded else str(x)
                   for x in date.split("-")]
    ...
    async def _get_last_comic_number(self, comic_data, date=None):
        ...
        date_dict = self._date_dict(dt.datetime.today().strftime(
            "%m-%d-%Y") if date == None else date, padded=comic_data.get("padded", True))
        try:
            archive_url = comic_data["archive_url"].format(
                *[date_dict[x] for x in comic_data.get("archive_keys", [])])
            archive_html = await DL.async_text(archive_url)
        except:
            return (None, None)
        latest_comic = self._walk_replace(
            archive_html, comic_data["latest_url"])
        if not latest_comic:
            return (None, None)
    ...
    async def _get_random_comic(self, comic_data):
        ...
        if use_number:
            ...
            last = self._julian_day(comic_data.get(
                "last_date", dt.datetime.today().strftime("%m-%d-%Y")))
    ...
    def _walk_replace(self, search_text, steps, key_dict=None):
        ...
        for step in steps:
            try:
                if key_dict:
                    text = text.split(step["find"].format(
                        *[key_dict[x] for x in step.get("keys", [])]))[step["index"]]
                else:
    ...
    async def _get_comic(self, comic_data, date=None, latest_tuple=None):
        ...
        if comic_data.get("comic_number", False):
            ...
            if not isinstance(date, int):
                date_dict = self._date_dict(
                    date, padded=comic_data.get("padded", True))
                # We have a date to check for
                date = self._walk_replace(
                    archive_html, comic_data["date_url"], date_dict)
                if not date:
                    return None
                try:
                    date = int(date)
            ...
            last_date = comic_data.get("last_date", dt.datetime.today().strftime(
                "%m-%d-%Y"))  # Last supplied date, or today
            # Gather our julian days for comparison
            first_julian, last_julian, date_julian = [
                self._julian_day(x) for x in (first_date, last_date, date)]
            ...
            date_dict = self._date_dict(
                date, padded=comic_data.get("padded", True))
            url = comic_data["url"].format(
                *[date_dict[x] for x in comic_data["keys"]])
        try:
            html = await DL.async_text(url, {'User-Agent': ''})
        except:
...
#        h = html()
        ...
        comic_title = self._walk_replace(html, comic_data["comic_title"]) if len(
            comic_data.get("comic_title", [])) else comic_data["name"]
        if not comic_title:
            comic_title = comic_data["name"]
        comic_title += " ({}{})".format("#" if isinstance(date,
                                                          int) else "", date)
        comic_title = html.unescape(unquote(comic_title))
        # Check if we need to get a description
        comic_desc = self._walk_replace(html, comic_data["comic_desc"]) if len(
            comic_data.get("comic_desc", [])) else None
        comic_desc = html.unescape(unquote(comic_desc)) if comic_desc else None
    ...
    async def _display_comic(self, ctx, comic, date=None, random=False):
        ...
        message = await Message.EmbedText(
            ...
            description="Feeling around in the dark trying to find a {} comic...".format(
                self.comic_data[comic]["name"]),
            color=ctx.author
        ).send(ctx)
        if random:
            ...
            desc = "{} comic {}".format(self.comic_data[comic]["name"], date if isinstance(
                date, int) else "for today" if date == None else "for "+date)
            try:
