...
class Music(commands.Cog):
    ...
    def __init__(self, bot, settings):
        ...
        self.ll_uri = bot.settings_dict.get(
            "lavalink_rest_uri", "http://127.0.0.1:2333")
        self.ll_pass = bot.settings_dict.get(
    ...
    def dict_pop(self, ctx):
        ...
        guild = ctx if isinstance(ctx, discord.Guild) else ctx.guild if isinstance(
            ctx, discord.ext.commands.Context) else ctx.channel.guild if isinstance(ctx, discord.VoiceState) else None
    ...
    async def resolve_search(self, ctx, url, shuffle=False):
        ...
        if data == False:
            return  # Something else happened, ignore it.
        if data == None:
            # Nothing found
        ...
        if isinstance(data, wavelink.Track):
            ...
            await Message.Embed(
                ...
                fields=[
                            {"name": "Duration", "value": self.format_duration(
                                data.duration, data), "inline": False}
                ],
                color=ctx.author,
            ...
            await Message.EmbedText(
                title="♫ Added {}playlist: {} ({} song{})".format(
                    "shuffled " if shuffle else "", data.data["playlistInfo"]["name"], len(data.tracks), "" if len(data.tracks) == 1 else "s"),
                description="Requested by {}".format(ctx.author.mention),
                url=data.search,
                delete_after=delay,
                color=ctx.author
    ...
    async def add_to_queue(self, ctx, url, message, shuffle=False):
        ...
        if isinstance(tracks, wavelink.Track):
            ...
            try:
                seek_str = next((x[2:] for x in url.split("?")[1].split(
                    "&") if x.lower().startswith("t=")), "0").lower()
        ...
        if shuffle:
            random.shuffle(tracks.tracks)  # Shuffle before adding
    ...
    def progress_bar(self, player, track, bar_width=27, show_percent=True, include_time=False):
        ...
        if show_percent:
            bar += " --%" if total == 0 else " {}%".format(
                int(round(progress/total*100)))
        if include_time:
            time_prefix = "{} - {}\n".format(self.format_duration(
                progress), self.format_duration(total, track))
            bar = time_prefix + bar
        return bar
    ...
    def print_eq(self, eq, max_len=5):
        ...
        for band, value in eq:
            ...
            if value == 0:
                ...
                our_left = emp*max_len + sup + bar * \
                    (ourbar-1) + topleft + emp*(max_len-ourbar)
                our_cent = emp*max_len + sep + inner * \
                    (ourbar-1) + cap + emp*(max_len-ourbar)
                our_right = emp*max_len + sup + bar * \
                    (ourbar-1) + topright + emp*(max_len-ourbar)
            else:
                # Let's draw a bar going down
                our_left = emp*(max_len-ourbar) + botleft + \
                    bar*(ourbar-1) + sdn + emp*max_len
                our_cent = emp*(max_len-ourbar) + cap + inner * \
                    (ourbar-1) + sep + emp*max_len
                our_right = emp*(max_len-ourbar) + botright + \
                    bar*(ourbar-1) + sdn + emp*max_len
        ...
        graph = "```\n{}\n{}\n{}\n{}\n{}\n{}\n{}\n```".format(
                ...
                nums,
                sep*(len(nums)),
                "\n".join(["{}{}{}".format(lpad, x, lpad)
                           for x in map("".join, zip(*eq_list))]),
    ...
    async def on_event_hook(self, event):
        ...
        if isinstance(event, (wavelink.TrackEnd, wavelink.TrackException, wavelink.TrackStuck)):
            ...
            delay = self.settings.getServerStat(
                ctx.guild, "MusicDeleteDelay", 20)
            if isinstance(event, (wavelink.TrackException, wavelink.TrackStuck)):
                ...
                # Sends the TrackStop event to move to the next song as needed
    ...
    async def on_play_next(self, player, track):
        ...
        if seek and not seek > track.duration:
            await player.seek(track.info["seek"]*1000)
    ...
    async def on_next_song(self, ctx, error=None):
        ...
        if player.is_playing or player.is_paused:
            return await player.stop()  # This will fire another "next_song" event so we bail here
        # Gather up the queue
        queue = self.queue.get(str(ctx.guild.id), [])
        ...
        volume = self.vol[str(ctx.guild.id)] if str(
            ctx.guild.id) in self.vol else self.settings.getServerStat(ctx.guild, "MusicVolume", 100)
        eq = wavelink.eqs.Equalizer.build(levels=self.settings.getServerStat(
            ctx.guild, "MusicEqualizer", wavelink.eqs.Equalizer.flat().raw))
        if not player.volume == volume/2:
            await player.set_volume(volume/2)
        if not player.eq.raw == eq.raw:
        ...
        await Message.Embed(
            ...
            description="Requested by {}".format(
    ...
    async def _load_playlist_from_url(self, url, ctx, shuffle=False):
        ...
        if not len(playlist):
            return await Message.EmbedText(title="♫ Playlist is empty!", color=ctx.author).edit(ctx, message)
        if not isinstance(playlist, list):
            return await Message.EmbedText(title="♫ Playlist json is incorrectly formatted!", color=ctx.author).edit(ctx, message)
        if shuffle:
            random.shuffle(playlist)
        # Let's walk the items and add them
        queue = self.queue.get(str(ctx.guild.id), [])
        for x in playlist:
            if not "id" in x or not isinstance(x["id"], str):
                continue  # id missing or not string - skip
    ...
    async def playing(self, ctx, *, moons=None):
        ...
        delay = self.settings.getServerStat(ctx.guild, "MusicDeleteDelay", 20)
        player = self.bot.wavelink.players.get(
            ctx.guild.id, None) if ctx.guild.id in self.bot.wavelink.players else None
        ...
        data = self.data.get(str(ctx.guild.id))
        play_text = "Playing" if (
            player.is_playing and not player.is_paused) else "Paused"
        cv = int(player.volume*2)
        await Message.Embed(
            title="♫ Currently {}: {}".format(play_text, data.title),
            description="Requested by {} -- Volume at {}%".format(
                data.info["added_by"].mention, cv),
            color=ctx.author,
            fields=[
                {"name": "Elapsed", "value": self.format_elapsed(
                    player, data), "inline": False},
                {"name": "Progress", "value": self.progress_moon(player, data) if moons and moons.lower(
                ) in ["moon", "moons", "moonme", "moon me"] else self.progress_bar(player, data), "inline":False}
            ],
            url=data.uri,
            thumbnail=data.thumb,
            delete_after=delay
    ...
    async def playingin(self, ctx):
        ...
        for x in self.bot.wavelink.players:
            ...
            if p.is_playing and not p.is_paused:
                server_list.append(
                    {"name": server.name, "value": p.current.info.get("title", "Unknown title")})
        msg = "♫ Playing music in {:,} of {:,} server{}.".format(
            len(server_list), len(self.bot.guilds), "" if len(self.bot.guilds) == 1 else "s")
        if len(server_list):
    ...
    async def playlist(self, ctx):
        ...
        if len(queue):
            ...
            if total_time:
                # Got time at least
                time_string += "{} total -- ".format(
                    self.format_duration(total_time))
            if total_streams:
                # Got at least one stream
                time_string += "{:,} Stream{} -- ".format(
                    total_streams, "" if total_streams == 1 else "s")
            q_text = "-- {:,} Song{} in Queue -- {}".format(
                len(queue), "" if len(queue) == 1 else "s", time_string)
            fields.append(
                {"name": "♫ Up Next", "value": q_text, "inline": False})
    ...
    async def unskip(self, ctx):
        ...
        if not ctx.author.id in skips:
            return await Message.EmbedText(title="♫ You haven't voted to skip this song!".format(len(new_skips), needed_skips), color=ctx.author, delete_after=delay).send(ctx)
    ...
    async def seek(self, ctx, position=None):
        ...
        vals = ["0"] * (len(multiplier)-len(vals)) + vals if len(
            vals) < len(multiplier) else vals  # Ensure we have 3 values
        for index, mult in enumerate(multiplier):
            try:
                # Try to avoid h, m, s suffixes
                seconds += mult * \
                    float(
                        "".join([x for x in vals[index] if x in "0123456789."]))
            except:
                return await Message.EmbedText(title="♫ Malformed seek value!", description="Please make sure the seek time is in seconds, or using HH:MM:SS format.", color=ctx.author, delete_after=delay).send(ctx)
        ms = int(seconds*1000)
    ...
    async def volume(self, ctx, volume=None):
        ...
        try:
            volume = float(volume)
            volume = int(volume) if volume - \
                int(volume) < 0.5 else int(volume)+1
        except:
        ...
        self.settings.setServerStat(
            ctx.guild, "MusicVolume", 10 if volume < 10 else 100 if volume > 100 else volume)
    ...
    async def seteq(self, ctx, *, bands=None):
        ...
        if not len(band_ints) == 15:
            return await Message.EmbedText(title="♫ Incorrect number of eq values! ({} - need 15)".format(len(band_ints)), description="15 numbers separated by a space from -5 (silent) to 5 (double volume)", color=ctx.author, delete_after=delay).send(ctx)
        eq_list = [(x, float(0.25 if y/20 > 0.25 else -0.25 if y /
                             20 < -0.25 else y/20)) for x, y in enumerate(band_ints)]
    ...
    async def setband(self, ctx, band_number=None, value=None):
        ...
        new_bands = [(band_number-1, float(value/20)) if x ==
                     band_number-1 else (x, y) for x, y in player.eq.raw]
    ...
    async def eqpreset(self, ctx, preset=None):
        ...
        eq = wavelink.eqs.Equalizer.boost() if preset.lower() == "boost" else wavelink.eqs.Equalizer.flat(
        ) if preset.lower() == "flat" else wavelink.eqs.Equalizer.metal()
        await player.set_eq(eq)
        player._equalizer = eq  # Dirty hack to fix a bug in wavelink
        eq_text = self.print_eq(player.eq.raw)
        self.settings.setServerStat(ctx.guild, "MusicEqualizer", player.eq.raw)
    ...
    async def cog_before_invoke(self, ctx):
        ...
        if ctx.command.name in ("playingin", "autodeleteafter", "disableplay", "stopall", "searchlist", "playing", "playlist"):
            return
        ...
        if ctx.command.name in ("join", "summon"):
            return
        ...
        if player == None or not player.is_connected:
            ...
            raise commands.CommandError(
                "Music Cog: Not connected to a voice channel.")
        # Let's make sure the caller is connected to voice and the same channel as the bot - or a bot-admin
        if Utils.is_bot_admin(ctx):
            return  # We good - have enough perms to override whatever
        if not ctx.author.voice or player == None or not ctx.author.voice.channel.id == int(player.channel_id):
            ...
            raise commands.CommandError(
                "Music Cog: Author not connected to the bot's voice channel.")
